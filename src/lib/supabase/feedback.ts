...
/** Submit feedback; uses RPC insert_feedback when available, else direct insert. */
export async function submitFeedback(feedback: FeedbackInsert): Promise<FeedbackRecord> {
  try {
    ...
    if (!functionError && functionData) {
      ...
      if (result) return result as FeedbackRecord;
    }
  } catch {
...
export async function getUserFeedback(userId?: string, userAddress?: string): Promise<FeedbackRecord[]> {
  ...
  else if (userAddress) query = query.eq('user_address', userAddress);
  else throw new Error('Either userId or userAddress must be provided');
