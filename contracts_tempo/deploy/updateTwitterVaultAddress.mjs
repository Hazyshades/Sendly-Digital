...
const TWITTER_VAULT_ADDRESS = process.env.VITE_ARC_TWITTER_VAULT_ADDRESS;
const GIFT_CARD_ADDRESS = process.env.VITE_ARC_CONTRACT_ADDRESS;

if (!RPC_URL || !PRIVATE_KEY || !TWITTER_VAULT_ADDRESS || !GIFT_CARD_ADDRESS) {
  console.error('❌ Ошибка: Отсутствуют необходимые переменные окружения');
  console.error('Требуются: VITE_ARC_RPC_URL, DEPLOYER_PRIVATE_KEY, VITE_ARC_TWITTER_VAULT_ADDRESS, VITE_ARC_CONTRACT_ADDRESS');
  process.exit(1);
}

const TWITTER_VAULT_ABI = [
  {
    "inputs": [
      {
...
async function updateTwitterVaultAddress() {
  try {
    console.log('🔄 Обновление адреса GiftCard в TwitterCardVault...\n');
    
    const provider = new ethers.JsonRpcProvider(RPC_URL);
    const wallet = new ethers.Wallet(PRIVATE_KEY, provider);
    
    console.log('📋 Параметры:');
    console.log(`   TwitterCardVault: ${TWITTER_VAULT_ADDRESS}`);
    console.log(`   Новый GiftCard: ${GIFT_CARD_ADDRESS}`);
    console.log(`   Отправитель: ${wallet.address}\n`);
    
    const twitterVaultContract = new ethers.Contract(TWITTER_VAULT_ADDRESS, TWITTER_VAULT_ABI, wallet);
    
    console.log('⏳ Отправка транзакции setGiftCardContract...');
    
    const tx = await twitterVaultContract.setGiftCardContract(GIFT_CARD_ADDRESS);
    ...
    if (receipt.status === 1) {
      console.log('✅ Адрес GiftCard успешно обновлен в TwitterCardVault!\n');
