...
const TIKTOK_VAULT_ADDRESS =
  process.env.VITE_ARC_TIKTOK_VAULT_ADDRESS ||
  process.env.VITE_ARC_TIKTOK_VAULT_CONTRACT_ADDRESS;

if (!RPC_URL || !PRIVATE_KEY || !GIFT_CARD_ADDRESS || !TIKTOK_VAULT_ADDRESS) {
  console.error('❌ Ошибка: Отсутствуют необходимые переменные окружения');
  console.error('Требуются: VITE_ARC_RPC_URL, DEPLOYER_PRIVATE_KEY, VITE_ARC_CONTRACT_ADDRESS, VITE_ARC_TIKTOK_VAULT_ADDRESS/VITE_ARC_TIKTOK_VAULT_CONTRACT_ADDRESS');
  process.exit(1);
}
...
async function linkTikTokVault() {
  try {
    console.log('🔗 Связывание TikTokCardVault с GiftCard...\n');
    ...
    console.log(`   TikTokCardVault: ${TIKTOK_VAULT_ADDRESS}`);
    console.log(`   Отправитель: ${wallet.address}\n`);

    const giftCardContract = new ethers.Contract(GIFT_CARD_ADDRESS, GIFT_CARD_ABI, wallet);

    console.log('⏳ Отправка транзакции setTikTokVaultContract...');

    const tx = await giftCardContract.setTikTokVaultContract(TIKTOK_VAULT_ADDRESS);
    ...
    if (receipt.status === 1) {
      console.log('✅ TikTokCardVault успешно связан с GiftCard!\n');
    ...
    if (error.code === 'CALL_EXCEPTION') {
      console.error('   Возможно, контракт GiftCard не имеет установленного TikTok Vault или есть другая ошибка вызова.');
